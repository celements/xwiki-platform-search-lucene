...
/**
 * This plugin allows index based search in the contents of Wiki Pages and their
 * attachments, as far as any text can be extracted from them. Text can be extracted from
 * OpenOffice Writer, MSWord, PDF, XML/XHTML, plain text, etc. Text extraction is done
 * with the help of various third party libs such as Apache POI and PDFBox and some
 * classes from the Daisy project.
 * <p>
...
public class LucenePluginApi extends PluginApi<LucenePlugin> {
  ...
  /**
   * Return value for {@link #rebuildIndex()} meaning that the caller does not have admin
   * rights.
   */
  public static final int REBUILD_NOT_ALLOWED = -1;

  /**
   * Return value for {@link #rebuildIndex()} meaning that another rebuild is already in
   * progress.
   */
  ...
  /**
   * Searches the configured Indexes using the specified lucene query for documents in the
   * given languages.
   * <p>
   * With virtual wikis enabled in your xwiki installation this will deliver results from
   * all virtuall wikis. For searching in a subset of your virtual wikis see
   ...
   *          <ul>
   *          <li><code>default</code> for content having no specific language information
   *          </li>
   ...
   *          query entered by the user
   * @param sortField
   *          field to use to sort the results list (ex: date, author)
   * @param languages
   *          comma separated list of language codes to search in, may be null to search
   *          all languages. Language codes can be:
   *          <ul>
  ...
  /**
   * Searches the configured Indexes using the specified lucene query for documents in the
   * given languages belonging to one of the given virtual wikis.
   * <p>
   ...
   *          Names of the virtual wikis to search in. May be null for global search.
   * @param languages
   *          comma separated list of language codes to search in, may be null to search
   *          all languages. Language codes can be:
   *          <ul>
  ...
  public SearchResults getSearchResults(String query, String[] sortField, String virtualWikiNames,
    ...
    try {
      ...
      LOGGER.error(
          "Failed to search: query=[{}], sortField=[{}], virtualWikiNames=[{}], languages=[{}]",
